  ...
  */
  ...
  if ( empty($name) || empty($roll) || empty($phone) || empty($email) || empty($pass_one) || empty($pass_two)  ) {
    ...
    <script type="text/javascript">
      window.location.href = "../student-signup.php?signup=empty&name=$name&roll=$roll&phone=$phone&email=$email"
    </script>
    <?php
    exit();
  }
  ...
  if ($resultcheck>0) {
    while ($row = mysqli_fetch_assoc($result)) {
      ...
      if ($email == $row['st_mail']) {
        ...
        ?>
<script type="text/javascript">
  window.location.href = "../student-signup.php?signup=again&name=$name&roll=$roll&phone=$phone&email=$email"
</script>
<?php
        exit();
      }
  ...
  //if password doesn't match
   if( $pass_one != $pass_two ) {
    ...
    ?>
    <script type="text/javascript">
  window.location.href = "../student-signup.php?signup=not_matched&name=$name&roll=$roll&phone=$phone&email=$email"
</script>
<?php
      exit();
    }
    ...
    $st_roll = ($roll[4]*100)+($roll[5]*10)+$roll[6];
      ...
      if ( strlen($roll)!=7 || $st_roll<1 || $st_roll>$total_roll ) {
        // header("Location: ../student-signup.php?signup=invalied-roll&name=$name&roll=$roll&phone=$phone&email=$email");
            ?>
    <script type="text/javascript">
  window.location.href = "../student-signup.php?signup=invalied-roll&name=$name&roll=$roll&phone=$phone&email=$email"
</script>
<?php
        exit();
      }
      ...
      if ($resultcheck>0) {
        // header("Location: ../student-signup.php?signup=duplicate&name=$name&roll=$roll&phone=$phone&email=$email");
                    ?>
    <script type="text/javascript">
  window.location.href = "../student-signup.php?signup=duplicate&name=$name&roll=$roll&phone=$phone&email=$email"
</script>
<?php
        exit();
      }
    ...
    //student signing in
      ...
      $sql = "INSERT INTO student_data(st_roll, st_name, st_pass, st_series, st_dept, is_pending, st_mail, st_phone) VALUES('$roll', '$name', '$en_pass', '$series', '$dept_name', 0, '$email', '$phone');";
    ...
    //   header("Location: ../student_pending.php");
        ?>
            <script type="text/javascript">
        //   window.location.href = "../student_pending.php"
